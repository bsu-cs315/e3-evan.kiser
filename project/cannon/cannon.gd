...
const _MIN_HORIZONTAL_ANGLE := deg_to_rad(-90)
const _MAX_HORIZONTAL_ANGLE :=deg_to_rad(90)
...
func _process(delta: float) -> void:
	...
	if angle_vertical_axis != 0:
		...
		$CannonMesh.rotation.x += angle_vertical_axis * angle_change_rate * delta
		angle_vertical = clampf(angle_vertical, _MIN_VERTICAL_ANGLE, _MAX_VERTICAL_ANGLE)
		$CannonMesh.rotation.x = clampf(angle_vertical, _MIN_VERTICAL_ANGLE, _MAX_VERTICAL_ANGLE)
	...
	if angle_horizontal_axis != 0:
		...
		$CannonMesh.rotation.y += angle_horizontal_axis * angle_change_rate * delta
		angle_horizontal = clampf(angle_horizontal, _MIN_HORIZONTAL_ANGLE, _MAX_HORIZONTAL_ANGLE)
		$CannonMesh.rotation.y = clampf(angle_horizontal, _MIN_HORIZONTAL_ANGLE, _MAX_HORIZONTAL_ANGLE)
